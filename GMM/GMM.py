...
class GMMModel:
    ...
    def _e_step(self, X):
        ...
        for i in range(self.n_components):
            ...
            responsibilities[i] = self.weights[i] * exponent
    ...
    def _m_step(self, X, responsibilities):
        ...
        for i in range(self.n_components):
            self.means[i] = (
                torch.sum(responsibilities[i].unsqueeze(1) * X, dim=0) / total_weight[i]
            )
            diff = X - self.means[i]
            self.covariances[i] = (
                torch.matmul(
                    (diff * responsibilities[i].unsqueeze(1)).transpose(0, 1), diff
                )
                / total_weight[i]
            )
